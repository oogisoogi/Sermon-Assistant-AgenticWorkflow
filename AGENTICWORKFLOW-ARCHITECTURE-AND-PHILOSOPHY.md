...
graph TD
    ...
    B --> C["<b>Sermon Assistant</b><br/>(실제 구현된 자식)<br/>DNA: 동일 / GRA + P1 Master 강발현"]
...
#### 실제 구현된 자식 시스템: Sermon Assistant

**Sermon Assistant**는 AgenticWorkflow에서 분화된 최초의 자식 시스템이다. 설교 연구 도메인에 특화되면서, 부모의 전체 게놈을 구조적으로 내장한다:

| 부모 DNA | Sermon Assistant에서의 발현 |
|---------|--------------------------|
| 절대 기준 1 (품질) | 11개 박사급 연구 에이전트, GRA 3계층 품질 보증, SRCS 4축 평가 |
| 절대 기준 2 (SOT) | state.yaml(글로벌) + session.json(설교별 하위 디렉터리) 2계층 SOT |
| P1 할루시네이션 봉쇄 | `_sermon_lib.py` 2,984줄 결정론적 라이브러리 + P1 Master 함수 패턴 |
| 4계층 품질 보장 | Cross-Validation Gates + SRCS + GroundedClaim 스키마 |
| Safety Hook | Hallucination Firewall ("모든 학자가 동의" 패턴 차단) |
| 번역 프로토콜 | `@sermon-translator` (신학 전문 번역 + theological-glossary.yaml) |
| Context Preservation | `/sermon-resume` (도메인 특화 복구 — session.json + translation_state) |

상세: [`SERMON-ASSISTANT-ARCHITECTURE-AND-PHILOSOPHY.md`](SERMON-ASSISTANT-ARCHITECTURE-AND-PHILOSOPHY.md)
